...
import { collectVulnerabilities, NpmAudit, runNpmAudit } from "../utils/npmAudit";
...
export async function auditPluggableWidgetsTools(fix: boolean = false) {
    ...
    const vulnerabilities = collectVulnerabilities(report, report.vulnerabilities[pluggableWidgetsTools])
    const vulnerableDependencies = vulnerabilities
        .map(v => v.name)
        .reduce((unique, p) => unique.includes(p) ? unique : [...unique, p], [] as NpmAudit.PackageName[]);
    const updateable = await Promise.all(vulnerableDependencies
        .map(name => report.vulnerabilities[name])
        .map(findSafeVersion));
    ...
    if (updateablePwt?.safeRange) {
        ...
        console.log(bold(whiteBright("Update @mendix/pluggable-widgets-tools to at least %s")), updateablePwt.safeRange)
        return
    }

    // Report vulnerable dependencies
    console.log(yellow("Found %d dependencies with %d vulnerabilities"), vulnerableDependencies.length, vulnerabilities.length)
    ...
    if (updateable.some(p => p.safeRange) && (fix || await confirm("Add overrides to package.json for vulnerable packages?"))) {
...
function reportOtherDirectVulnerabilities(report: NpmAudit.Report) {
    if (Object.values(report.vulnerabilities).some(p => p.isDirect && p.name != pluggableWidgetsTools)) {
        console.log(yellow("Other dependencies of the widget contain vulnerabilities. Run `npm audit` for more information."))
    }
}
...
async function findSafeVersion({ name, range }: NpmAudit.Dependency): Promise<UpdateablePackage> {
    ...
    const versions = await promisify(exec)(`npm show ${name} versions --json`).then(({ stdout }) => JSON.parse(stdout) as string[]);
