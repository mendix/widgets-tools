...
function getGHRepoAuthUrl(repoUrl: string): string {
    ...
    if (!GH_USERNAME || !GH_PAT) {
        throw new ConfigurationError("Required GH_USERNAME and GH_PAT env vars are not set.");
    }
...
async function setLocalGitUserInfo(workingDirectory?: string): Promise<void> {
    ...
    if (!GH_NAME || !GH_EMAIL) {
        throw new ConfigurationError("Required GH_NAME and GH_EMAIL env vars are not set.");
    }
